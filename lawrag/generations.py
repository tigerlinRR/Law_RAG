...
def _local(ts: datetime | None) -> str | None:
    """Render a timezone-aware timestamp in the computer's local time. The DB
    session runs in UTC, so formatting in SQL would show UTC; converting here
    with .astimezone() (no arg = system local tz) makes History match the clock
    on this machine."""
    return ts.astimezone().strftime("%Y-%m-%d %H:%M") if ts else None
...
def list_for(allowed_clients: list[str] | None) -> list[dict]:
    ...
    with db.connect() as conn, conn.cursor() as cur:
        cur.execute(f"""
            SELECT id, kind, source_name, client, item, created_by, created_at
            FROM generations {where}
            ORDER BY created_at DESC
        """, params)
        return [
            {"id": r[0], "kind": r[1], "source_name": r[2], "client": r[3],
             "item": r[4], "created_by": r[5], "created_at": _local(r[6])}
...
def get(gen_id: int, allowed_clients: list[str] | None) -> dict | None:
    ...
    with db.connect() as conn, conn.cursor() as cur:
        cur.execute("""
            SELECT id, kind, source_name, client, item, created_by, created_at, result
    ...
    return {"id": row[0], "kind": row[1], "source_name": row[2], "client": row[3],
            "item": row[4], "created_by": row[5], "created_at": _local(row[6]),
            "result": row[7]}
